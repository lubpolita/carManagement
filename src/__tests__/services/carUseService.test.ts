...
describe('CarUse Service', () => {
    ...
    let findByIdCarUseService: FindByIdService
    ...
    beforeEach(async () => {
      ...
      endCarUseService = new EndCarUseService(fakeCarUseRepository)
        findByIdCarUseService = new FindByIdService(fakeCarUseRepository)
        findAllCarUseByDriverService = new FindAllCarUseByDriverService(fakeCarUseRepository)
      findAllCarUseService = new FindAllCarUseService(fakeCarUseRepository)
    })

    it('Should be able to create a new car use', async () => {
    
       const car = await fakeCarRepository.create({ brand: 'brand test',
                                                    color: 'green',
                                                    licensePlate: 'AAAA'
       })
        
       const car2 = await fakeCarRepository.create({ brand: 'brand test',
       color: 'blue',
       licensePlate: 'AAAA'
   })
       ...
       const driver = await fakeDriverRepository.create({ name: "Maria"})
        ...
        const act = await createCarUseService.execute({
            carId: car.id, driverId: driver.id, startDate: '12-03-2021', reason: 'reason',
            finalDate: ''
        })
        expect(act).toBeInstanceOf(CarUse) 
        
       await expect( async () => { await createCarUseService.execute({
        carId: car.id, driverId: driver.id, startDate: '12-03-2021', reason: 'reason',
        finalDate: ''})
       }).rejects.toThrow(Error('Car is busy'))
        
       await expect( async () => { await createCarUseService.execute({
        carId: car2.id, driverId: driver.id, startDate: '12-03-2021', reason: 'reason',
        finalDate: ''})
    ...
    it('Should be able to end car use', async () => {
        ...
        await expect( async () => { await endCarUseService.execute(carUse.id)
        }).rejects.toThrow(Error('Implossible terminate a contract that has already been terminated.'))
        
        await expect( async () => { await endCarUseService.execute('')
        }).rejects.toThrow(Error('Please, enter a valid Id'))

        await expect( async () => { await endCarUseService.execute('id_error')
    ...
    it('Should be able to find by a driver id a car use', async () => {
        const carUse = await fakeCarUseRepository.create({
            carId: 'carid', driverId: 'driverid', finalDate: '2023-04-02',
            startDate: '2023-04-01', reason: 'reason'})
