...
const findAllCarUseByDriverServiceMock = FindAllCarUseByDriverService as jest.MockedClass<
...
describe('CarUse', () => {
  ...
  afterAll(async () => {
    ...
    it('Should be able to create a new car use', async () => {
        ...
        await createCarUseServiceMock.prototype.execute.mockResolvedValueOnce(new CarUse(),);
    ...
    expect(response.body).not.toBe(undefined)
      ...
      it('Should be able to get all car uses', async () => {
        ...
        await findAllCarUseServiceMock.prototype.execute.mockResolvedValueOnce([ new CarUse() ]);
  ...
  it('Should be able to get a car use by id', async () => {
    ...
    await findByIdCarUseServiceMock.prototype.execute.mockResolvedValueOnce(new CarUse());
...
expect(response.status).toBe(201)
  ...
  it('Should be able to find a car use by driver id', async () => {
    ...
    await findAllCarUseByDriverServiceMock.prototype.execute.mockResolvedValueOnce([ new CarUse() ]);
